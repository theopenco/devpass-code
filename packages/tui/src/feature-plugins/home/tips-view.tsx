...
const TIPS: Tip[] = [
  ...
  "Create {highlight}devpass-code.json{/highlight} for server settings and {highlight}tui.json{/highlight} for TUI settings",
  "Place TUI settings in {highlight}~/.config/devpass-code/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.devpass-code/commands/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.devpass-code/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.devpass-code/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.devpass-code/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent DevPass Code from reading sensitive files",
  "Use {highlight}devpass-code run{/highlight} for non-interactive scripting",
  "Use {highlight}devpass-code --continue{/highlight} to resume the last session",
  "Use {highlight}devpass-code run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}devpass-code serve{/highlight} for headless API access to DevPass Code",
  "Use {highlight}devpass-code run --attach{/highlight} to connect to a running server",
  "Run {highlight}devpass-code upgrade{/highlight} to update to the latest version",
  "Run {highlight}devpass-code auth list{/highlight} to see all configured providers",
  "Run {highlight}devpass-code agent create{/highlight} for guided agent creation",
  "Run {highlight}devpass-code github install{/highlight} to set up the GitHub workflow",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.devpass-code/themes/{/highlight} directory",
  ...
  "Run {highlight}devpass-code debug config{/highlight} to troubleshoot configuration",
