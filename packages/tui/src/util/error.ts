...
export function cliErrorMessage(input: unknown): string | undefined {
  ...
  if (model) {
    ...
    return [
      ...
      "Try: `devpass-code models` to list available models",
      "Or check your config (devpass-code.json) provider/model names",
    ].join("\n")
  }
  ...
  if (isRecord(input) && named(input, "MCPFailed")) {
    ...
    return `MCP server "${name}" failed. Note, devpass-code does not support MCP authentication yet.`
  }
  return undefined
}
