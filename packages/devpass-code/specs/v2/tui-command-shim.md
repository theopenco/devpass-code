...
Delete `packages/devpass-code/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/devpass-code/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/devpass-code/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/devpass-code`
- TUI plugin loader tests in `packages/devpass-code` if runtime plugin API wiring changed
