...
- The server still lives in `packages/devpass-code`.
...
Until enough shared service code lives outside `packages/devpass-code`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/devpass-code`

It should not import `packages/devpass-code` services while `packages/devpass-code`
