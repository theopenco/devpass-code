...
function globalConfigFile() {
  const candidates = ["devpass-code.jsonc", "devpass-code.json", "opencode.jsonc", "opencode.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "devpass-code.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "devpass-code.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          for (const name of ["opencode", "devpass-code"]) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".opencode") || dir.endsWith(".devpass-code") || dir === Flag.OPENCODE_CONFIG_DIR) {
            for (const file of ["devpass-code.json", "devpass-code.jsonc", "opencode.json", "opencode.jsonc"]) {
