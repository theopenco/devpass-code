...
- Use `testEffect(...)` from `packages/devpass-code/test/lib/effect.ts` for tests that exercise Effect services, layers, runtime context, scoped resources, or platform integrations.
...
- Run tests from package directories such as `packages/devpass-code`; never run package tests from the repo root.
